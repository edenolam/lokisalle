...
// ------------- Affichage du panier ------------
	...
	for($i = 0; $i < count($_SESSION['panier']['produit']); $i++){
		...
		if(connecte()){
				...
				<div id="panierform">
				<form method='post'>
				<a href='?action=vider'>*vider mon panier<hr style="color: #c4c4c4; width: 400px; float: left;"></a>
	...
	if(connecte()){
        echo '<p>Votre adresse de facturation : ' . $_SESSION['utilisateur']['adresse'] . " - " . $_SESSION['utilisateur']['cp'] . " - " . $_SESSION['utilisateur']['ville'];'</p>';
    }
 	echo "</div>";
